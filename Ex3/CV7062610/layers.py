...
def batchnorm_backward(dout, cache):
    ...
    dsample_mean = np.sum(dx_norm * -1.0 / np.sqrt(sample_var + eps), axis=0) + dsample_var * np.mean(-2.0 * (x - sample_mean), axis=0)

    
    dx = dx_norm * 1.0 / np.sqrt(sample_var + eps) + dsample_var * 2.0 / N * (x - sample_mean) + dsample_mean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(di * x_norm, axis=0)
    dbeta = np.sum(di, axis=0)
    dx_norm = di * gamma

    # backpropagate through standardization
    dvar = np.sum(dx_norm * x_centered, axis=0) * (-0.5) * ((sample_var + eps) ** (-1.5))
    dmean = np.sum(dx_norm, axis=0) * (-1 / std) + dvar * np.mean(-2 * x_centered, axis=0)

    # backpropagate through mean subtraction
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    cache = (x, x_normalized, mean, var, gamma, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    # still apply!                                                            #
    ###########################################################################
    x, x_normalized, mean, var, gamma, eps = cache
    ...
    dvar = np.sum(dx_normalized * (x - mean) * (-0.5) * np.power(var + eps, -1.5), axis=1, keepdims=True)
